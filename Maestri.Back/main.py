...
from Controller.HookController import HookController
...
hook_controller = HookController(hook)
payment_controller = PaymentController(pay)
...
@app.route('/ok2')
def get_pix2():
    result = payment_controller.create_pix_payment()
    return ""

@app.route('/ok3')
def get_pix_slip():
    result = payment_controller.create_pix_slip_payment()
    return "sin"

@app.route("/hook_pix")
def hook_pix():
    response = hook_controller.create_hook_pix()
    return "hook_pix"

@app.route("/hook_pix_slip")
def hook_pix_slip():
    response = hook_controller.create_hook_pix_slip()
